...
export function ActivityStepper({ flow, currentCode, labelFor, testID }: ActivityStepperProps) {
    ...
    const currentIndex = flow.findIndex((a) => a.code === currentCode);
    const scrolls = flow.length > COMPACT_AT;

    const steps = flow.map((activity, i) => {
        // An unrecognised current status leaves every step "upcoming" rather
        // than falsely marking progress.
...
export function nextActivity(flow: FlowActivity[], currentCode?: string | null): FlowActivity | undefined {
    const i = flow.findIndex((a) => a.code === currentCode);
    if (i < 0) return flow[0];
    if (flow[i]?.complete) return undefined;
    return flow[i + 1];
}
