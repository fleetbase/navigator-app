...
interface OdometerDigitProps {
    ...
    digitWrapperStyle?: StyleProp<ViewStyle>;
    digitContainerStyle?: StyleProp<ViewStyle>;
}

interface OdometerNumberProps extends Omit<OdometerDigitProps, 'digit'> {
    value?: number;
 ...
 */
const OdometerDigit = ({ digit, digitHeight = 30, duration = 300, digitStyle, digitWrapperStyle, digitContainerStyle }: OdometerDigitProps) => {
    // Only the setter is used — the wheel animates from `digit` directly.
    const [, setCurrentDigit] = useState(digit);
 ...
 */
const OdometerNumber = ({ value = 0, digitHeight = 30, duration = 300, containerStyle, digitStyle, digitWrapperStyle, digitContainerStyle }: OdometerNumberProps) => {
    // No negative numbers.
    // v2 had `value = value ? 0 : value` here — inverted, so ANY non-zero value
    // rendered as 0. That is why the v2 dashboard's odometer tiles always showed 0.
